...
class Utility():
    ...
    def getGeneralHelpString(self) -> str:
        ...
        _result += "\n" + helpFlags + ": Prints this information about input arguments."
        _result += "\n" + testFlags + ": A method of calling experimental code (when you want to test if something works)."
        _result += "\n" + editFlags + " [playlistId or index: str]: Opens the file with Playlist."
        _result += "\n" + searchFlags + " [searchTerm: str] [? includeSoftDeleted: bool]: Search all Playlists, QueueStreams, and StreamQueues, uri and names where available. Supports Regex."
    ...
    def getPlaylistHelpString(self) -> str:
        ...
        _result = ""
        _result += "\n" + addPlaylistFlags + " [name: str] [? playWatchedStreams: bool] [? allowDuplicates: bool] [? streamSourceIds: list]: Add a Playlist with name: name, playWatchedStreams: if playback should play watched QueueStreams, allowDuplicates: should Playlist allow duplicate QueueStreams (only if the uri is the same), streamSourceIds: a list of StreamSources."
        _result += "\n" + addPlaylistFromYouTubeFlags + " [youTubePlaylistUrl: str] [? name: str] [? playWatchedStreams: bool] [? allowDuplicates: bool]: Add a Playlist and populate it with QueueStreams from given YouTube playlist youTubePlaylistUrl, with name: name, playWatchedStreams: if playback should play watched streams, allowDuplicates: should Playlist allow duplicate QueueStreams (only if the uri is the same)."
        _result += "\n" + deletePlaylistFlags + " [playlistIds or indices: list]: deletes Playlists indicated."
        _result += "\n" + restoreSourceFlags + " [playlistIds or index: str]: restore soft deleted Playlist from database."
        _result += "\n" + listPlaylistFlags + " [? includeSoftDeleted: bool]: List Playlists with indices that can be used instead of IDs in other commands."
        _result += "\n" + detailsPlaylistFlags + " [playlistIds or indices: list] [? enableFetch: bool] [? enableFetch: bool]: Prints details about given playlist, with option for including StreamSources and QueueStreams."
        _result += "\n" + fetchPlaylistSourcesFlags + " [playlistIds or indices: list] [? takeAfter: datetime] [? takeBefore: datetime]: Fetch new streams from StreamSources in Playlists indicated, e.g. if a Playlist has a YouTube channel as a source, and the channel uploads a new video, this video will be added to the Playlist. Optional arguments takeAfter: only fetch QueueStreams after this date, takeBefore: only fetch QueueStreams before this date. Dates formatted like \"2022-01-30\" (YYYY-MM-DD)."
        _result += "\n" + prunePlaylistFlags + " [playlistIds or indices: list]: Prune Playlists indicated, deleteing watched QueueStreams."
        _result += "\n" + purgePlaylistFlags + " [? includeSoftDeleted: bool] [? permanentlyDelete: bool]: Purge database indicated, removing IDs with no corresponding relation and deleteing StreamSources and QueueStreams with no linked IDs in Playlists."
        _result += "\n" + resetPlaylistFetchFlags + " [playlistIds or indices: list]: Resets fetch status of StreamSources in a Playlist and deletes QueueStreams from Playlist."
        _result += "\n" + playFlags + " [playlistId or index: str] [? starindex: int] [? shuffle: bool] [? repeat: bool]: Start playing stream from a Playlist, order and automation (like skipping already watched QueueStreams) depending on the input and Playlist."
        _result += self.getPlaylistSwitchesHelpString()
        
        return _result
    
    def getPlaylistSwitchesHelpString(self) -> str:
        """
        Get the Playlist-section switches of help-print as a string.

        Returns:
            str: a string of Playlist switches info
        """

        _result = ""
        _result += "\n\t" + quitSwitches + ": End current playback and contintue the program without playing anymore QueueStreams in Playlist. Only available while Playlist is playing."
        _result += "\n\t" + skipSwitches + ": Skip current QueueStream playing. This QueueStream will not be marked as watched. Only available while Playlist is playing."
        _result += "\n\t" + addCurrentToPlaylistSwitches + " [playlistId or index: str]: Add the current QueueStream to another Playlist indicated by ID on index. Only available while Playlist is playing."
        _result += "\n\t" + printPlaybackDetailsSwitches + ": Prints details of current playing Playlist."
    ...
    def getQueueStreamHelpString(self) -> str:
        ...
        _result = ""
        _result += "\n" + addStreamFlags + " [playlistId or index: str] [uri: string] [? name: str]: Add a stream to a Playlist from ID or index, from uri: URL, and name: name (set automatically if not given)."
        _result += "\n" + deleteStreamFlags + " [playlistId or index: str] [streamIds or indices: list]: delete QueueStreams from Playlist.")
        _result += "\n" + restoreStreamFlags + " [playlistId or index: str] [streamIds or indices: str]: restore soft deleted QueueStreams from database.")
    ...
    def getStreamSourceHelpString(self) -> str:
        ...
        _result = ""
        _result += "\n" + addSourcesFlags + " [playlistId or index: str] [uri: string] [? enableFetch: bool] [? name: str]: Add a StreamSources from uri: URL, enableFetch: if the Playlist should fetch new stream from this StreamSource, and name: name (set automatically if not given)."
        # _result += "\n" + addSourcesFlags + " [playlistId or index: str] [uri: string] [? enableFetch: bool] [? backgroundContent: bool] [? name: str]: Add a StreamSources from uri: URL, enableFetch: if the Playlist should fetch new QueueStream from this StreamSource, backgroundContent; if the QueueStream from this source are things you would play in the background, and name: name (set automatically if not given)."
        _result += "\n" + deleteSourceFlags + " [playlistId or index: str] [sourceIds or indices: str]: deletes StreamSources from database and Playlist if used anywhere."
        _result += "\n" + restoreSourceFlags + " [playlistId or index: str] [sourceIds or indices: str]: restore soft deleted StreamSources from database."
        _result += "\n" + listSourcesFlags + " [? includeSoftDeleted: bool]: Lists StreamSources with indices that can be used instead of IDs in other commands."
    ...
    def getMetaHelpString(self) -> str:
        ...
        _result = ""
        _result += "\n" + listSettingsFlags + ": Lists settings currently used by program. These settings can also be found in the file named \".env\" with examples in the file \".env-example\"."
        _result += "\n" + listSoftDeletedFlags + " [? simplified: bool]: Lists all soft deleted entities. Option for simplified, less verbose list."
